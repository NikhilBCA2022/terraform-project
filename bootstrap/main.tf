...
resource "aws_s3_bucket_public_access_block" "state_replica_logs" {
  provider                = aws.replica
...
resource "aws_s3_bucket_server_side_encryption_configuration" "state_replica_logs" {
  provider = aws.replica
  bucket   = aws_s3_bucket.state_replica_logs.id
...
# ---------------------------------------------------------------------------
# S3 — access logging config for state bucket
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# S3 — access logging + lifecycle for replica bucket
# ---------------------------------------------------------------------------
resource "aws_s3_bucket_logging" "state_replica" {
  provider      = aws.replica
  bucket        = aws_s3_bucket.state_replica.id
  target_bucket = aws_s3_bucket.state_replica_logs.id
  target_prefix = "state-replica-access-logs/"
}
